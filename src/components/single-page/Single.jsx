...
const Single = ({
  ...
  const playlist =
    playlistData.find((item) => item.id === id) ||
    playlistData2.find((item) => item.id === id) ||
    playlistData7.find((item) => item.id === id);
  return (
    <div className="w-full">
      <Layout>
        <div className="all3 w-full h-[550px] p-10 relative">
            ...
            <div className="custom-div2 mt-[100px] ml-8">
              ...
              <p className="p2 font-bold">
                {playlist?.name?.length > 9
                  ? `${playlist?.name.substring(0, 9)}...`
                  : playlist?.name}
              </p>
              <p className="text-[20px] mt-5">{playlist?.description}</p>
            </div>
          </div>
            ...
            <table className="table">
              <thead className="thead">
                <tr className="tr">
                  ...
                  <th className="px-4 py-2 time">
                    <img src={Union} alt="" />
                  </th>
                </tr>
              </thead>
              <tbody>
                {playlistData5.map((item, index) => (
                  <tr key={index} className="tir">
                    <td className=" px-4 py-2 td1">{index + 1}</td>
                    <td className="td2 relative">
                      <button
                        className="flex items-center play_btn2 absolute top-9 rounded-[50%] bg-green-500 justify-center p-3"
                        onClick={() => playTrack(index)}
                      >
                        {isPlaying && currentTrackIndex === index ? (
                          <FaPause className="text-black text-2xl" />
                        ) : (
                          <img src={Start} alt="" className="text-xl" />
                        )}
                      </button>
                    </td>
                    ...
                    {/* Assuming item.album contains the album name */}
